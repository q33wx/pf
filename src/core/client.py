...
class SolanaClient:
    ...
    async def get_latest_blockhash(self) -> Hash:
        ...
        await self._rate_limiter.acquire()
        client = await self.get_client()
        # "confirmed" is more reliable across Helius / multi-node RPCs than
        # "processed" (avoids BlockhashNotFound on sim/preflight).
    ...
    async def confirm_transaction(
        ...
        """
        status = await self.confirm_transaction_status(signature, commitment)
        return status == "ok"

    async def confirm_transaction_status(
        self,
        signature: str,
        commitment: str = "confirmed",
        timeout_seconds: float = 45.0,
    ) -> str:
        """Wait for a transaction and report its landed status.

        Polls getSignatureStatuses (works with plain string signatures — the
        solana-py confirm_transaction helper requires a Signature object and
        crashes on str, which previously made every landed buy look failed).

        Returns:
            ``"ok"``       — landed and executed successfully
            ``"failed"``   — landed but the program errored (meta.err set)
            ``"unknown"``  — not observed before timeout; caller MUST verify
                             on-chain state before assuming it didn't land.
        """
        sig_str = str(signature)
        want = {"finalized", "confirmed"}
        ...
        while asyncio.get_event_loop().time() < deadline:
            body = {
                ...
                "params": [[sig_str], {"searchTransactionHistory": True}],
            }
            response = await self.post_rpc(body)
            info = None
            if response and "result" in response:
                values = (response["result"] or {}).get("value") or [None]
                info = values[0]
            if info:
                if info.get("err"):
                    logger.error(
                        f"Transaction {sig_str[:16]}... landed but failed: "
                        f"{info['err']}"
                    )
                    return "failed"
                if (info.get("confirmationStatus") or "") in want:
                    return "ok"
            await asyncio.sleep(1.5)

        logger.warning(
            f"Transaction {sig_str[:16]}... not confirmed within "
            f"{timeout_seconds:.0f}s — status unknown (may still land)"
        )
        return "unknown"
