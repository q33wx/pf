...
# Fishing mode: single-mint volume fisher (no new-token sniping filters)
FISHING_REQUIRED_FIELDS = [
    "name",
    "rpc_endpoint",
    "wss_endpoint",
    "private_key",
...
def is_fishing_mode(config: dict) -> bool:
    """Return True if this config runs the single-mint fishing strategy."""
    return config.get("mode", "snipe") == "fishing"


def validate_config(config: dict) -> None:
    """Validate the configuration against defined rules with platform support."""
    # Default mode for backward compatibility with sniper YAML files
    if "mode" not in config:
        config["mode"] = "snipe"

    fishing = is_fishing_mode(config)
    required = FISHING_REQUIRED_FIELDS if fishing else REQUIRED_FIELDS

    # Validate required fields
    for field in required:
        get_nested_value(config, field)

    # Validate config rules (skip sniper-only paths that may be absent in fishing)
    ...
    # Fishing-mode specific checks
    if fishing:
        fishing_cfg = config.get("fishing", {})
        mint = fishing_cfg.get("mint")
        if not mint or not isinstance(mint, str) or len(mint) < 32:
            raise ValueError("fishing.mint must be a valid base58 Solana mint address")
        venue = fishing_cfg.get("venue", "pumpswap")
        if venue not in ("pumpswap", "bonding_curve"):
            raise ValueError(
                "fishing.venue must be 'pumpswap' (migrated AMM) or "
                "'bonding_curve' (pre-migration)"
            )
        scale_levels = fishing_cfg.get("scale_out_levels")
        if scale_levels is not None:
            if not isinstance(scale_levels, list) or not scale_levels:
                raise ValueError(
                    "fishing.scale_out_levels must be a non-empty list of "
                    "{pnl_pct, sell_pct} entries"
                )
            for i, level in enumerate(scale_levels):
                if "pnl_pct" not in level or "sell_pct" not in level:
                    raise ValueError(
                        f"fishing.scale_out_levels[{i}] needs pnl_pct and sell_pct"
                    )
                if not (0 < float(level["sell_pct"]) <= 1):
                    raise ValueError(
                        f"fishing.scale_out_levels[{i}].sell_pct must be in (0, 1]"
                    )

    # Platform-specific validation (sniper listener rules only for snipe mode)
    ...
    try:
        ...
        if not fishing:
            ...
            # Still check platform is registered
            try:
                ...
                if not platform_factory.registry.is_platform_supported(platform):
                    raise ValueError(
                        ...
                        f"Available: {[p.value for p in platform_factory.get_supported_platforms()]}"
                    )
            except ImportError:
                pass
    except ValueError as e:
...
def print_config_summary(config: dict) -> None:
    ...
    if mode == "fishing":
        fishing = config.get("fishing", {})
        print(f"Fishing mint: {fishing.get('mint', 'not set')}")
        print(
            f"  - Volume window: {fishing.get('volume_window_seconds', 30)}s | "
            f"min net buy: {fishing.get('min_net_buy_sol', '?')} SOL"
        )
        print(f"  - Dry run: {fishing.get('dry_run', True)}")
    else:
        print(
        ...
        try:
            ...
            if listener_type and not validate_platform_listener_combination(
                ...
                print(
                    f"WARNING: Listener '{listener_type}' may not be compatible "
                    f"with platform '{platform_str}'"
                )
        except ValueError:
    ...
    buy_slip = trade.get("buy_slippage")
    if buy_slip is not None:
        print(f"  - Buy slippage: {buy_slip * 100}%")
    if mode != "fishing":
        print(
            f"  - Extreme fast mode: "
            f"{'enabled' if trade.get('extreme_fast_mode') else 'disabled'}"
        )
